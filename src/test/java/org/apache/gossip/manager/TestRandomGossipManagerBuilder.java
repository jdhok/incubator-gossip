...
public class TestRandomGossipManagerBuilder {
  ...
  public void useMemberListIfProvided() {
    ...
    for (GossipMember aMember : gossipManager.getMemberList()) {
      if (aMember == gossipManager.getMyself()) {
        continue;
      }
      LocalGossipMember actual = gossipManager.getMemberList().get(0);
      Assert.assertEquals(member.getId(), actual.getId());
    }
  }
